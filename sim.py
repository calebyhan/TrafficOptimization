...
class Sim:
    ...
    def unpack(self):
        ...
        for sub in ret:
            if len(sub) < max_arrays:
                sub.extend([[np.nan, np.nan, np.nan, np.nan, np.nan]] * (max_arrays - len(sub)))
...
class Road:
    ...
    def run(self):
        if np.random.rand() < abs(self.x[self.t]):
            self.add_car()
...
class Car:
    ...
    def __init__(self):
        ...
        self.position = 0
        self.idle = 0
        self.turn = np.random.choice([0, 1, 2], p=[0.3, 0.3, 0.4])
        print(self.turn)
